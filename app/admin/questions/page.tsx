...
import { Pencil, Trash2, Plus, Download, Search as SearchIcon } from "lucide-react"
    ...
    <div className="space-y-6">
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border p-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h1 className="text-2xl font-bold bg-linear-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">题目管理</h1>
            <p className="text-muted-foreground text-sm mt-1">管理和维护所有学科的题目库</p>
          </div>
          <div className="flex flex-wrap gap-3">
             <div className="flex bg-slate-100 rounded-lg p-1">
              ...
              <Button 
                ...
                variant="ghost" 
                size="sm" 
                className="h-8 text-xs hover:bg-white hover:shadow-sm text-muted-foreground"
              >
                全部停用
              </Button>
            </div>
            ...
            <Button onClick={() => (window.location.href = "/admin/questions/add")} className="h-10 shadow-lg shadow-primary/20">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6 items-center">
          <div className="relative flex-1 min-w-[300px]">
            ...
            <Input 
              ...
              className="pl-9 h-10 bg-slate-50/50 border-slate-200 focus-visible:ring-1 transition-all"
            />
          </div>
          <div className="flex flex-wrap gap-2 items-center flex-1 justify-end">
             <Select value={type} onValueChange={(v) => setType(v || undefined)}>
              <SelectTrigger className="w-[110px] h-10 bg-white border-dashed"><SelectValue placeholder="题型" /></SelectTrigger>
              <SelectContent>
             ...
             <Select value={difficulty} onValueChange={(v) => setDifficulty(v || undefined)}>
              <SelectTrigger className="w-[100px] h-10 bg-white border-dashed"><SelectValue placeholder="难度" /></SelectTrigger>
              <SelectContent>
             ...
             <Select value={grade} onValueChange={(v) => setGrade(v || undefined)}>
              <SelectTrigger className="w-[100px] h-10 bg-white border-dashed"><SelectValue placeholder="年级" /></SelectTrigger>
              <SelectContent>
             ...
             <Select value={subject} onValueChange={(v) => setSubject(v || undefined)}>
              <SelectTrigger className="w-[90px] h-10 bg-white border-dashed"><SelectValue placeholder="科目" /></SelectTrigger>
              <SelectContent>
            ...
            <Button onClick={() => { setPage(1); fetchQuestions() }} size="icon" variant="secondary" className="h-10 w-10 shrink-0">
               <SearchIcon className="w-4 h-4" />
            </Button>
            {(keyword || type || difficulty || grade || subject || isActive) && (
              <Button 
                onClick={() => {
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => (window.location.href = `/admin/questions/edit/${question.id}`)}
                      >
