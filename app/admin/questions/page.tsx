...
export default function AdminPage() {
  ...
  const handleBulkStatus = async (active: boolean) => {
    if (!confirm(active ? "确定要启用全部题目吗？" : "确定要停用全部题目吗？"))
      return;
    try {
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6 items-center">
          ...
          <div className="flex flex-wrap gap-2 items-center flex-1 justify-end">
            <Select value={type} onValueChange={(v) => setType(v || undefined)}>
              <SelectTrigger className="w-[110px] h-10 bg-white border-dashed">
            ...
            <Select
              value={difficulty}
              onValueChange={(v) => setDifficulty(v || undefined)}
            >
              <SelectTrigger className="w-[100px] h-10 bg-white border-dashed">
            ...
            <Select
              value={grade}
              onValueChange={(v) => setGrade(v || undefined)}
            >
              <SelectTrigger className="w-[100px] h-10 bg-white border-dashed">
            ...
            >
              <SelectTrigger className="w-[90px] h-10 bg-white border-dashed">
          ...
          <TableBody>
            {questions.map((question) => (
              <TableRow key={question.id}>
                ...
                <TableCell className="max-w-lg truncate text-left">
                  {renderMathContent(
                    (question.description || "").replace(
                      /\{_([0-9]+)\}/g,
                      "()"
                    ),
                    `admin-list-desc-${question.id}`,
                    { as: "span", justify: "start" }
                  )}
                </TableCell>
                <TableCell>
                  <Badge
                ...
                <TableCell>
                  <Badge variant="outline">
                    {question.grade === "grade1"
                      ? "一年级"
                      : question.grade === "grade2"
                      ? "二年级"
                      : question.grade === "grade3"
                      ? "三年级"
                      : question.grade === "grade4"
                      ? "四年级"
                      : question.grade === "grade5"
                      ? "五年级"
                      : question.grade === "grade6"
                      ? "六年级"
                      : "-"}
                  </Badge>
                </TableCell>
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-2">
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        (window.location.href = `/admin/questions/edit/${question.id}`)
                      }
                    >
                    ...
                    <Button
                      ...
                      className={
                        question.is_active ? "text-amber-600" : "text-green-600"
                      }
                    >
